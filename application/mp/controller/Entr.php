...
class Entr
{
    ...
    public function keyword($keyword, $msg = [])
    {
        $rule = Db::name('mp_rule')->where(['mpid' => $this->mid, 'keyword' => $keyword, 'status' => '1'])
            ->where('event', '')
            ->order('id Desc')->find();
        $rulecount = Db::name('mp_rule')->where(['mpid' => $this->mid, 'keyword' => $keyword, 'status' => '1'])
            ->where('event', '')
            ->count();
            ...
            switch ($rule['type']) {//text,addon,images,news,voice,music,video
                ...
                case 'news'://图文
                    ...
                    if ($rulecount == 1) {
                        ...
                        $rulegroup = Db::name('mp_rule')
                            ->field('GROUP_CONCAT(reply_id) as reply_id')
                            ->where(['mpid' => $this->mid, 'keyword' => $keyword, 'status' => '1'])
                            ->where('event', '')
                            ->group('keyword')
                            ->find();
                        $content = Db::name('mp_reply')->where('reply_id','in',$rulegroup['reply_id'])->select();
